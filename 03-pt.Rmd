...
From KARS app (module *Portugal*) we can simulate the rules from the PT model, setting the scores as we pleased. On sidebar panel, default punctuation (figure \@ref(fig:ptpoints)) is presented but the user can mess with it and apply new set of scores to order waiting list of candidates for a given donor.
...
When selected ption **Multiple donors** (figure \@ref(fig:ptmultdonor)) the user just have to click on *'Select your options an run it!'* and wait for the results.
